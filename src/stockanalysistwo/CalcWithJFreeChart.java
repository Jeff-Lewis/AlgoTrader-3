 ...
 * @author garth
 */
public class CalcWithJFreeChart extends ApplicationFrame {
    
    public CalcWithJFreeChart(XYSeriesCollection dataset,final String title) {
    ...
    //special method :p
    public static double OptimiseParameters(double minSRDifin,int curvePointsin,int periodin,double difPercentin , double TP, double[] PortfolioValue, double[] BaseValue,XYSeriesCollection dataset) 
    {        
		...
		try {
			int lineNumber = findNumDays(fileName);
                        ...
                        for(int i =0;i<lineNumber-1;i++)
                        {
                            ...
                            lowPrices[lineNumber-1-i]= Double.parseDouble(stringArray[2]);           //initialise array of daily lows
